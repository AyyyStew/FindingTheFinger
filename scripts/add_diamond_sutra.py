"""
scripts/add_diamond_sutra.py

Migrates the Diamond Sutra (Gemmell) from corpus.duckdb into postgres.

Hierarchy:
    height=1  Section  — "Section 1", "Section 2", ...  (30)
    height=0  Passage  — "DS 1.1", "DS 2.1", ...        (93)

The text has 30 sections numbered 1-32 (sections 3 & 4 absent in this
translation), matching the traditional 32-section structure.

Run from project root:
    python -m scripts.add_diamond_sutra
"""
...
def main():
    ...
    with Session(engine) as session:
        ...
        # Levels
        for height, name in [(1, "Section"), (0, "Passage")]:
        ...
        passages = old.execute("""
            SELECT p.id, CAST(p.section AS INTEGER), p.unit_number, p.text
        ...
        section_units: dict[int, int] = {}  # section_num -> unit.id
        old_to_new:    dict[int, int] = {}

        print("Inserting units...")
        for old_id, section_num, unit_number, text_ in passages:

            # Section (depth=0)
            if section_num not in section_units:
                u = Unit(
                    corpus_id=corpus.id,
                    parent_id=None,
                    depth=0,
                    label=f"Section {section_num}",
                    meta={"section": section_num},
                )
                session.add(u)
                session.flush()
                section_units[section_num] = u.id

            # Passage (depth=1)
            u = Unit(
                corpus_id=corpus.id,
                parent_id=section_units[section_num],
                depth=1,
                label=f"DS {section_num}.{unit_number}",
                text=text_,
            )
            session.add(u)
            session.flush()
        ...
        print(f"  {len(section_units)} sections, {len(old_to_new)} passages")
