...
Migrates the Bhagavad Gita from corpus.duckdb into postgres.

Hierarchy:
    height=1  Chapter  — "Arjun Viṣhād Yog", ...  (18)
    height=0  Shloka   — "Arjun Viṣhād Yog 1.1"   (640)

Run from project root:
    python -m scripts.add_gita
"""
...
def main():
    ...
    with Session(engine) as session:
        ...
        # Levels
        for height, name in [(1, "Adhyaya"), (0, "Shloka")]:
        ...
        for old_id, book, unit_label, text_ in passages:
            if book not in chapter_units:
                u = Unit(corpus_id=corpus.id, parent_id=None, depth=0, label=book)
                session.add(u)
                session.flush()
                chapter_units[book] = u.id

            u = Unit(
                corpus_id=corpus.id,
                parent_id=chapter_units[book],
                depth=1,
                label=unit_label,
                text=text_,
            )
        ...
        print(f"  {len(chapter_units)} chapters, {len(old_to_new)} shlokas")
