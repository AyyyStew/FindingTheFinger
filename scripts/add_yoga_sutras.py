"""
scripts/add_yoga_sutras.py

Migrates the Yoga Sutras of Patanjali (Johnston) from corpus.duckdb into postgres.

Hierarchy:
    height=1  Pada   — "Samadhi Pada", "Sadhana Pada", "Vibhuti Pada", "Kaivalya Pada"  (4)
    height=0  Sutra  — "YS 1.1", "YS 1.2", ...                                          (193)

Old DB used "Book I–IV" and redundant section prefix in labels — both rebuilt here.

Run from project root:
    python -m scripts.add_yoga_sutras
"""
...
CORPUS_NAME  = "Yoga Sutras of Patanjali (Johnston)"
MODEL        = "all-mpnet-base-v2"
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://ftf:ftf@localhost:5432/ftf")

# Map old "Book I–IV" to traditional Pada names and their numbers
PADA_MAP = {
    "Book I":  (1, "Samadhi Pada"),
    "Book II": (2, "Sadhana Pada"),
    "Book III":(3, "Vibhuti Pada"),
    "Book IV": (4, "Kaivalya Pada"),
}
...
def main():
    ...
    with Session(engine) as session:
        ...
        # Levels
        for height, name in [(1, "Pada"), (0, "Sutra")]:
        ...
        passages = old.execute("""
            ...
            WHERE c.name = ?
            ORDER BY p.book, p.unit_number
        """, [CORPUS_NAME]).fetchall()
        print(f"  {len(passages)} sutras")

        pada_units: dict[str, int] = {}
        old_to_new: dict[int, int] = {}

        print("Inserting units...")
        for old_id, book, unit_number, text_ in passages:
            pada_num, pada_name = PADA_MAP[book]

            if book not in pada_units:
                u = Unit(
                    corpus_id=corpus.id,
                    parent_id=None,
                    depth=0,
                ...
                session.add(u)
                session.flush()
                pada_units[book] = u.id

            u = Unit(
                corpus_id=corpus.id,
                parent_id=pada_units[book],
                depth=1,
                label=f"YS {pada_num}.{unit_number}",
                text=text_,
            )
            session.add(u)
            session.flush()
        ...
        print(f"  {len(pada_units)} padas, {len(old_to_new)} sutras")
