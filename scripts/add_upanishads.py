"""
scripts/add_upanishads.py

Migrates the Upanishads (Paramananda) from corpus.duckdb into postgres.

Hierarchy:
    height=1  Upanishad  — "Isa Upanishad", "Katha Upanishad", "Kena Upanishad"  (3)
    height=0  Mantra     — "Isa Upanishad 1", "Isa Upanishad 2", ...              (198)

Note: old DB labels used section.verse format (e.g. "1.1") but section is
always 1, so we rebuild labels from unit_number directly.

Run from project root:
    python -m scripts.add_upanishads
"""
...
CORPUS_NAME  = "Upanishads (Paramananda)"
...
def main():
    ...
    with Session(engine) as session:
        ...
        # Levels
        for height, name in [(1, "Upanishad"), (0, "Mantra")]:
        ...
        print(f"  {len(passages)} mantras")

        upanishad_units: dict[str, int] = {}
        old_to_new:      dict[int, int] = {}

        print("Inserting units...")
        for old_id, book, unit_number, text_ in passages:
            if book not in upanishad_units:
                u = Unit(corpus_id=corpus.id, parent_id=None, depth=0, label=book)
                session.add(u)
                session.flush()
                upanishad_units[book] = u.id

            u = Unit(
                corpus_id=corpus.id,
                parent_id=upanishad_units[book],
                depth=1,
                label=f"{book} {unit_number}",
                text=text_,
            )
            session.add(u)
            session.flush()
        ...
        print(f"  {len(upanishad_units)} upanishads, {len(old_to_new)} mantras")
